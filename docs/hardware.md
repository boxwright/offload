...
| The same box, **a second model, the default install**: upstream `server-cuda` image, `install/compose.yaml` as shipped, speculative decoding off (no draft head) | **Qwen3-Coder-30B-A3B-Instruct**, Unsloth UD-Q4_K_XL (17.7 GB, Apache-2.0; mixture of experts, 3.3B active of 30.5B) | **measured** 2026-09-24 in a maintenance window (`docs/evidence/window-2/`) | model ready 11 s from a warm disk cache; GPU memory **20.8 GB at 65k context, 24.2 GB at 131k**; decode **299 tok/s (prose), 301 tok/s (code)**; prompt 1,940 tok/s (code); a three-turn tool task through sandbox → proxy → model in 6.8 s; calibration **5/5 in 274 s** (10 to 22 turns per task; the same tasks took Qwen3.8-27B 75 s in 4 to 9 turns: faster decode, more turns); one real job (`jobs/window-001`) done: local 34 s over 2 sessions, Claude 20 s, $0.15 |
