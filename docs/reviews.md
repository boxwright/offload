...
- ~~**A7 owner answers from Discord**~~ Built 2026-09-24 by dogfood job `a7-discord-inbox` ($0.82, no rescue, 180 tests) as a REST poll with a bot token (`docs/design/owner-answers-from-chat.md`, option C). Proven live 2026-09-26 (`jobs/j20260926-082131`): the owner's `Yes` in the channel woke the job in one poll interval; one crash in the poll (a message after the last gate was answered) found and fixed the same hour.
- **Owner branches of 2026-09-24, reviewed 2026-09-26:** `j20260924-082225` (spec `checks:`) merged as is; `j20260924-082543` (readable ids) was built on the other session's main with the parked reads branch inside, so its good parts (`slugify`, `resolve_job_id`, prefix lookup, wider status column, its tests) were taken by hand and the branch itself was not merged. Live: `j20260926-fix-the-failing-statistics-tests`.
